...
#Utilisation de opencv pour récuperer l'image 

method_lbp=['default',
			'ror',
			'uniform',
			'var']
#recuperer l'image d'un camera avec Videocapture
...
while True:
	ret, frame=cap.read() #Cap.read pour recuperer l'image

	image=cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) #Transformer l'image en noir et blanc
	lbp=feature.local_binary_pattern(image, numPoints, radius, method=method_lbp[id_method_lbp]) #récuperer la fonction from sckit image
